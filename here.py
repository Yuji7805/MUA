...
def verifyEmail(emailGetter):
    ...
    action = ActionChains(emailGetter)

    # Move to coordinate (1, 1) on the screen and click
    action.move_by_offset(1, 1).click().perform()
    email_element = wait.until(EC.visibility_of_element_located(
        (By.XPATH, "/html/body/section[2]/div/div/div[2]/div/div[2]/div[2]/div/div/div[1]/a")))    
    email_element.click()
    action.move_by_offset(1, 1).click().perform()
    iframe_first = emailGetter.find_element(
        By.XPATH, "/html/body/section[2]/div/div/div[2]/div/div[2]/div[3]/iframe")
    emailGetter.switch_to.frame(iframe_first)
    sleep(5)
    verifiedUrl = emailGetter.find_element(
        By.XPATH, "/html/body/table/tbody/tr/td/div/table[1]/tbody/tr/td/table/tbody/tr/td/table/tbody/tr/td/table/tbody/tr[5]/td/table/tbody/tr/td/div/a").get_attribute("href")
...
def addEducation(driver, education):
    ...
    if expFlag:
        driver.find_element(
            By.XPATH, "/html/body/div[5]/div/div[2]/div/div[1]/div[2]/div/div[4]/div/div[2]/div[1]/a/div/button").click()
    else:
        waitInfinite(lambda: driver.execute_script(
            'document.querySelectorAll("button.air3-btn.air3-btn-secondary.air3-btn-circle")[1].click()'))
...
def configLast(driver, country, street, city, zipcode, phone, photo):
    ...
    waitInfinite(lambda: driver.find_element(By.CSS_SELECTOR,
                 'input[aria-labelledby="postal-code-label"]').send_keys(zipcode))
    waitInfinite(lambda: driver.find_element(By.CSS_SELECTOR,
...
wait.until(EC.visibility_of_element_located(
    (By.XPATH, "/html/body/section[1]/div[2]/div/div/div[1]/div[1]/button"))).click()
...
driver.find_element(
    By.ID, "first-name-input").send_keys(random.choice(profile['first_name']))
driver.find_element(
    By.ID, "last-name-input").send_keys(random.choice(profile['last_name']))
...
waitInfinite(lambda: driver.find_element(
    By.ID, "onetrust-close-btn-container").click())
...
driver.execute_script(
    'document.querySelectorAll("span.up-checkbox-replacement-helper")[1].click()')
...
cur = conn.cursor()
cur.execute("INSERT INTO accounts (email, name, status, timestamp) VALUES (?, ?, ?, ?)",
            (tempURL, name, 'active', str(datetime.now())))
conn.commit()
