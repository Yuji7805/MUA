    ...
    for i in range(num):
        try:
            process = subprocess.Popen(
                ['python', 'here.py', name], stdout=subprocess.PIPE)
...
def open():
    ...
    cur = conn.cursor()
    cur.execute(
        'SELECT email from accounts WHERE name=? AND status="active"', (name, ))
    email = cur.fetchone()
    cur.execute('UPDATE accounts SET status=? WHERE email=?',
                ('sent', email[0], ))
    print(email[0])
    conn.commit()
    conn.close()
    ...
    process = subprocess.Popen(
        ['python', 'google.py', email[0], url], stdout=subprocess.PIPE)
    output, error = process.communicate()
...
def change_status():
    ...
    cur = conn.cursor()
    for i in range(len(ids)):
        cur.execute('UPDATE accounts SET status=? WHERE no=?',
                    (status, ids[i],))
    conn.commit()
    conn.close()
...
def screenshot():
    image_files = [f for f in os.listdir('static') if f.endswith(
